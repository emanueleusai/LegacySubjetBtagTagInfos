...
BTagEffHistsHadTop::BTagEffHistsHadTop(const char* name) : BaseHists(name)
{
    // named default constructor
}

BTagEffHistsHadTop::~BTagEffHistsHadTop()
{
    // default destructor, does nothing
}

void BTagEffHistsHadTop::Init()
{
...
void BTagEffHistsHadTop::Fill()
{
    ...
    for (unsigned int i =0; i<bcc->topjets->size(); ++i) {
      ...
      if(!(jettagged&&(deltaphi>(2*PI/3))&&(bcc->topjets->at(i).pt()>150.)&&(fabs(bcc->topjets->at(i).eta())<2.5))) continue;
...
void BTagEffHistsHadTop::Finish()
{
