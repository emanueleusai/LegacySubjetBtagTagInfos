...
ExampleHists2::ExampleHists2(const char* name) : BaseHists(name)
{
  // named default constructor
   
}

ExampleHists2::~ExampleHists2()
{
  // default destructor, does nothing
}

void ExampleHists2::Init()
{
 ...
 /* double* logMET_bins = MakeLogBinning(40, 50, 1200);
  ...
  histo_nbin.push_back(100); histo_minvalue.push_back(-0.01); histo_maxvalue.push_back(0.3);
  ...
  histo_nbin.push_back(100); histo_minvalue.push_back(0.); histo_maxvalue.push_back(50.);
  histo_type.push_back("subTrackMultiplicity"); histo_type_descr.push_back("track multiplicity of the subjet");
  histo_nbin.push_back(50); histo_minvalue.push_back(0.); histo_maxvalue.push_back(50.);
  histo_type.push_back("subTrackPt"); histo_type_descr.push_back("track transverse momentum");
  histo_nbin.push_back(100); histo_minvalue.push_back(0.); histo_maxvalue.push_back(50.);
  histo_type.push_back("weight"); histo_type_descr.push_back("weight");
  histo_nbin.push_back(100); histo_minvalue.push_back(0.); histo_maxvalue.push_back(.1);

  histo_type.push_back("MassTop"); histo_type_descr.push_back("Top Mass");
  histo_nbin.push_back(100); histo_minvalue.push_back(0.); histo_maxvalue.push_back(500.);
  //histo_type.push_back("subVertexNvtx"); histo_type_descr.push_back("N of secondary vertex");
  //histo_nbin.push_back(6); histo_minvalue.push_back(0.); histo_maxvalue.push_back(6.);
...
//find the pt-leading top-jet
  ...
  for (unsigned int topjet_counter=0;topjet_counter<topjets->size();topjet_counter++)
  {
    ...
    //topjets->at(topjet_counter).pt()
    if(jettagged&&(deltaphi>(2*PI/3))&&(topjets->at(topjet_counter).pt()>150.)&&(fabs(topjets->at(topjet_counter).eta())<2.5)&&(topjets->at(topjet_counter).pt()>_maxpt))
    {
      _maxpt=topjets->at(topjet_counter).pt();
      _theindex=topjet_counter;
    }
  }
  if (_theindex>-1)
  {
    ...
    tmps.assign("MassTop");tmps.append("_");tmps.append(histo_flavour.at(flavour_index));
    Hist(tmps.c_str())->Fill(topjets->at(_theindex).v4().M(),weight);
  }
  ...
  for (unsigned int topjet_counter=0;topjet_counter<topjets->size();topjet_counter++)
  {
	...
	{ 
	  ...
	  tmps.assign("weight");tmps.append("_");tmps.append(histo_flavour.at(flavour_index));
	  Hist(tmps.c_str())->Fill(weight);
	  
	  tmps.assign("subTrackMultiplicity");tmps.append("_");tmps.append(histo_flavour.at(flavour_index));
	  Hist(tmps.c_str())->Fill(topjets->at(topjet_counter).subTrackMomentum().at(subjet_counter).size(),weight);
	  ...
	  for(unsigned int track_counter=0;track_counter<topjets->at(topjet_counter).subTrackMomentum().at(subjet_counter).size();track_counter++)
	    {
	      ...
	      tmps.assign("subTrackPt");tmps.append("_");tmps.append(histo_flavour.at(flavour_index));
	      Hist(tmps.c_str())->Fill( topjets->at(topjet_counter).subTrackMomentum().at(subjet_counter).at(track_counter)/cosh( topjets->at(topjet_counter).subTrackEta().at(subjet_counter).at(track_counter) ) ,weight);
	    }
	  ...
	  //tmps.assign("subVertexNvtx");tmps.append("_");tmps.append(histo_flavour.at(flavour_index));
	  //Hist(tmps.c_str())->Fill(topjets->at(topjet_counter).subVertexNTracks().at(subjet_counter),weight);
	  //JTC
